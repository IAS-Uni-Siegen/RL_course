...
class GoddardEnv(gym.Env):
    ...
    def __init__(self, rocket=Default(), render_mode="human"):
        ...
        self.observation_space = gym.spaces.box.Box(
            ...
            high=np.array(
                [np.finfo(np.float32).max, np.finfo(np.float32).max, self._r.M0]
            ),
            dtype=np.float32,
        )
    ...
    def _observation(self, normalize=True):
        ...
        if normalize:
            return (state - np.array([self._r.V0, self._r.H0, self._r.M1])) / np.array(
                [0.14, 0.015, self._r.M0 - self._r.M1]
            )
        else:
            return state
    ...
    def render(self, mode="human"):
        ...
        def world_to_screen(x, y_val):
            return (int(500 / 2 + x * scale), int((y - y_val) * scale))
        ...
        pygame.draw.rect(
            self.surf,
            (77, 153, 77),
            pygame.Rect(g1[0], g1[1], g2[0] - g1[0], g2[1] - g1[1]),
        )
        ...
        pygame.draw.rect(
            self.surf,
            (153, 153, 153),
            pygame.Rect(pad1[0], pad1[1], pad2[0] - pad1[0], pad2[1] - pad1[1]),
        )
        ...
        pygame.draw.rect(
            self.surf,
            (80, 80, 80),
            pygame.Rect(r1[0], r2[1], r2[0] - r1[0], r1[1] - r2[1]),
        )
        ...
        pygame.draw.rect(
            self.surf,
            (204, 26, 36),
            pygame.Rect(f1[0], f2[1], f2[0] - f1[0], f1[1] - f2[1]),
        )
