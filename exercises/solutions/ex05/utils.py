def build_uturn_course(course_dim, inner_wall_dim):
    ...
    for i in range(course_dim[0]):
        if i < wall_up_bound:
            ...
            track_row = (
                "W" * (half_street_len // 2 + 1)
                + "W-"
                + "o" * (half_street_len - 1 + half_street_len // 2)
            )
        elif wall_up_bound <= i < wall_bottom_bound:
            track_row = (
                "W" * street_width + "W" * inner_wall_dim[1] + "o" * street_width
            )
        else:
            track_row = (
                "W" * (half_street_len // 2 + 1)
...
def build_rect_course(course_dim, inner_wall_dim):
    ...
    for i in range(course_dim[0]):
        if i < wall_up_bound:
            ...
            track_row = (
                "o" * street_width + "W" * inner_wall_dim[1] + "o" * street_width
            )
        else:
