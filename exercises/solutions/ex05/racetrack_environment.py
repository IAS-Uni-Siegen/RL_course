...
class RaceTrackEnv:
        ...
        if self.is_terminal_state():
            ...
            if self.lbl2int["+"] in projected_steps:
                ...
                if self.lbl2int["W"] in projected_steps:
                    s = self.reset()
                    if projected_steps.index(self.lbl2int["+"]) < projected_steps.index(
                        self.lbl2int["W"]
                    ):
    ...
    def _random_start_position(self):
        ...
        ### BEGIN SOLUTION
        self.position = np.array(random.choice(self.start_positions), dtype=np.int8)
        ### END SOLUTION
    ...
    def _load_course(self, course):
        ...
        for y in range(y_size):
            self.course[y, :] = np.array(
                [self.lbl2int[i] for i in course[y]], dtype=np.int8
            )

        self.start_positions = [
            (i, j) for i, j in zip(*np.where(self.course == self.lbl2int["-"]))
        ]
        ### END SOLUTION
...
class StochRaceTrackEnv(RaceTrackEnv):
    ...
    def step(self, action):
        ...
        if (
            action[0] == self.cam[0, prev_p[0], prev_p[1]]
            and action[1] == self.cam[1, prev_p[0], prev_p[1]]
        ):
