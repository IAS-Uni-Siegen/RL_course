...
\frame{\frametitle{Reference tracking with disturbance rejection}
	\begin{columns}[t,onlytextwidth]
		...
		\begin{column}{0.54\textwidth}
			\begin{minipage}[c]{\linewidth}
				\begin{itemize}
					\item Cont. state and action space
		...
		\begin{column}{0.54\textwidth}
			\begin{minipage}[c]{\linewidth}
				\begin{itemize}
					...
					\item<2-> Calculation of \textcolor{signalgreen}{feasible set} requires a model
					\item<3-> Training data can be utilized to \href{https://ei.uni-paderborn.de/en/lea/teaching/veranstaltungen/teaching/translate-to-english-systemidentifikation}{identify} model
					\item<4-> Here, recursive least squares (RLS) is applied
				\end{itemize}
			\end{minipage}
		\end{column}
	\end{columns}
...
\frame{\frametitle{Saftey shield based on feasible set - proof of concept (1)}
	\begin{columns}[t,onlytextwidth]
		...
		\begin{column}{0.54\textwidth}
			\begin{minipage}[c]{\linewidth}
				\begin{itemize}
					\item Three different approaches (five trainings run each)
					\item \textcolor{signalblue}{$\mathrm{DDPG}_\mathrm{}$}: Agent without safety shield
					\item<2-> \textcolor{signalred}{$\mathrm{DDPG}_\mathrm{SG}$}: Agent with safety shield using perfect a priori knowledge
					\item<3-> \textcolor{signalgreen}{$\mathrm{DDPG}_\mathrm{SG,RLS}$}: Agent with safety shield without a priori knowledge, identifying model using RLS
					\item<4-> Results in D. Weber et al., \textit{Safe Reinforcement Learning-Based Control in
		...
		\begin{column}{0.45\textwidth}
			\begin{minipage}[c]{\linewidth}
				\begin{itemize}
					...
					\item<2-> Sampling time of the plant system is $T_\text{s}=\SI{50}{\micro\second}$
					\item<3-> DQN inference, safeguarding and system identification must fit into $T_\text{s}$
					\item<4-> Source: M. Schenke et al., \textit{Finite-Set Direct Torque Control via Edge Computing-Assisted
		...
		\begin{column}{0.45\textwidth}
			\begin{minipage}[c]{\linewidth}
				\begin{itemize}
					...
					\item<2-> CPU must evaluate each neuron sequentially
					\item<3-> FPGA can evaluate each neuron at the same time
					\item<4-> Maximum number of parallel computations is limited
				\end{itemize}
			\end{minipage}
		\end{column}
	\end{columns}
		...
		\begin{column}{0.45\textwidth}
			\begin{minipage}[c]{\linewidth}
				\begin{itemize}
					...
					\item<2-> Communication between test bench and workstation is based on TCP/IP
					\item<3-> Backward pass is generic and has no time constraints $\rightarrow$ low application effort
				\end{itemize}
			\end{minipage}
		\end{column}
	\end{columns}
	...
	\begin{center}
		\begin{tikzpicture}
			\node[inner sep=0pt] (img) {
				\href{https://www.youtube.com/watch?v=hQ49Mc6LV78&t=13}{\includegraphics[width=0.7\textwidth]{fig/lec14/Youtube_preview.jpg}}
			};
		...
		\begin{column}{0.45\textwidth}
			\begin{minipage}[c]{\linewidth}
				\begin{itemize}
					...
					\item<2-> This means, the distinguishing details must be extracted in some way
					\item<3-> Usually, they can be retrieved from a larger set of observations
				\end{itemize}
			\end{minipage}
		\end{column}
		\hfill
		...
		\begin{column}{0.4\textwidth}
			\begin{minipage}[c]{\linewidth}
				\begin{itemize}
					...
					\item<2-> Generate context $\bm{z}$ with a fix set of observations $\rightarrow$ $\bm{z}=\text{const.}$
					\item<3-> Source: D. Jakobeit et al., \href{https://ieeexplore.ieee.org/abstract/document/10068250}{\textit{Meta-Reinforcement Learning-Based Current
