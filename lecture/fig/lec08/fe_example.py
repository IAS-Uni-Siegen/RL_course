...
X, y = make_blobs(centers=[[0, 0], [0, 0]], cluster_std=3, **blobs_params)

df = pd.DataFrame(X, columns=["X1", "X2"])

df = df.assign(
    r=lambda x: np.sqrt(x.X1**2 + x.X2**2), theta=lambda x: np.arctan(x.X2 / x.X1)
)
...
axes[0].scatter(
    time, OLS().fit(time, skewed_data).predict(time), s=3, c="orange", ls=":"
)
axes[1].scatter(time, np.log1p(skewed_data), s=3, c="magenta")
axes[1].scatter(
    time, OLS().fit(time, np.log1p(skewed_data)).predict(time), s=3, c="orange", ls=":"
)
for i, ax in enumerate(axes):
    ax.set_yticks([])
    ax.set_xticks([])
